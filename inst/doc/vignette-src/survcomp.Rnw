...
%\VignetteDepends{survival, ipred, prodlim, survivalROC, SuppDists, grid, rmeta, bootstrap, R (>= 2.3.0)}
%\VignetteSuggests{Hmisc, CPE, clinfun}
...
To compute the hazard ratio \cite{Cox1972} for each gene in each dataset, we have to call the \Rmethod{hazard.ratio()}) function. See \Rmethod{?hazard.ratio} for details. Before we compute the hazard ratio, we have to rescale the gene expression data for each dataset to a comparable scale, since the Affymetrix and Agilent microarrays have a different range of their gene expression, which would affect the hazard ratio computation. The following table gives an overview of the gene expression ranges in the six datasets that are included in \Robject{breastCancerData}:
...
Therefor we use the following function with \Rfunarg{q=0.05}.
...
The combined concordance indices for the six datasets are:

<<combineCindicesOutput,results=tex>>==
xtable(ccindex)
@

The combined log2 D indices for the six datasets are:
...
The combined log2 hazard ratios for the six datasets are:
...
<<survivalCurveAllDatasets,fig=TRUE,eps=FALSE>>==
...
<<survivalCurveAURKA,fig=TRUE,pdf=FALSE,eps=TRUE>>==
aurkaGs <- "AURKA"
aurkaGid <- 6790
...
%------------------------------------------------------------ 
The \Rpackage{SurvComp} package integrates functions for meta-analysis of risk-prediction models, e.g. for the concordance index or the D index. The following example shows the \Rfunction{cindex.comp.meta()} function \cite{Haibe-kains2008}. The result table shows the p-values representing the difference between the cindices of two genes using the cindices of all six datasets. For example, the cindex of the gene AURKA is with a p-value of 0.00001 significantly different from the cindex of the gene VEGF using the six datasets.

<<cindexcompmetaAnalysis,results=tex>>==
