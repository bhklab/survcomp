`mrmr.surv` <-
function(mydata, surv.time, surv.event, cl, weights, comppairs=10, strat, alpha=0.05, outx=TRUE, na.rm=FALSE) {
...
#### get cIndex for each variable in dataset with surv.time, surv.event ####
	for(ivar in 1:nvar){
		...
		se <- surv.event[cc.ix]
		if(msurv && sum(se) == 0) {
			warning(paste("\nno events, the concordance index cannot be computed for variable ", colnames(mydata)[ivar]," !"))	
			res_cIndex[ivar]<-NA
			is.correct<-FALSE
		}
		if(!msurv && length(unique(cl2)) == 1) {
			warning(paste("\nonly one class, the concordance index cannot be computed for variable", colnames(mydata)[ivar]," !"))	
			res_cIndex[ivar]<-NA
			is.correct<-FALSE
		}
		...
		if(N <= 1) {
			warning(paste("\nweights of observations are too small (sum should be > 1), the concordance index cannot be computed for variable", colnames(mydata)[ivar]," !"))	
			res_cIndex[ivar]<-NA
			is.correct<-FALSE
		}	
		...
		lenU <- length(ustrat)
		if(is.correct){
			res_cIndex[ivar] <- .Call("get_concordanceIndex_onevariable", as.integer(as.logical(msurv)), as.integer(ustrat), as.double(mydata[,ivar]),as.integer(cl2), as.double(st), as.integer(se), as.double(weights), as.integer(strat),as.integer(N), as.integer(as.logical(outx)), as.integer(lenS), as.integer(lenU))
		}
	}

	res_mrmr_cIndex <- .Call("mrmr_cIndex", data.matrix(mydata),as.integer(is.na(mydata)),as.double(res_cIndex),ncol(mydata), nrow(mydata), -1000)
	names(res_mrmr_cIndex)<-colnames(mydata)
