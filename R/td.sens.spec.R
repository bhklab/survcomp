...
function(cl, surv.time, surv.event, time, span=0, sampling=FALSE, na.rm=FALSE, ...) {
	...
	rr <- survivalROC::survivalROC.C(Stime=st, status=se, marker.fake, predict.time=time, span=span,  ...)
	#rr <- survivalROC::survivalROC(Stime=st, status=se, marker.fake, cut.values=mycutoff, predict.time=time, span=span, lambda=lambda, ...)
	...
	if(sampling) {
		...
		theta.foo1 <- function(x, cl, surv.time, surv.event, time, ...) {
			...
			rr <- survivalROC::survivalROC.C(Stime=surv.time, status=surv.event, marker.fake,  predict.time=time, span=span, ...)
			#rr <- survivalROC::survivalROC(Stime=surv.time, status=surv.event, marker.fake, cut.values=mycutoff, predict.time=time, span=span, lambda=lambda, ...)
